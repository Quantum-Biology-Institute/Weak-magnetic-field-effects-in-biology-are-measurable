...
#'Z_score_comparison' # ((control_data_3 - mu_control) / sigma_control) - ((control_data_2 - mu_control) / sigma_control)
#'Visibility' #(Q3 - Q2) / ((Q3 + Q2)) * 100  [-100, +100]
#'Relative_difference_nonabs' # (control_data_3 - control_data_2) / max(control_data_3, control_data_2) #[-100, 100]
#'Percentage_change' #((Q3 - Q2) / Q2) * 100

# my_test = 'Z_score_comparison'
# my_test_str = ' \n Z-score diff.'

my_test = 'Visibility'
my_test_str = ' \n visibility'

# my_test = 'Percentage_change'
# my_test_str = ' \n percentage change'

# my_test = 'Relative_difference_nonabs'
# my_test_str = ' \n relative diff.'


tests, variables, miny, maxy, yticks, my_lines, my_labels, my_units = SD.Setup_Delta(my_test)
miny_avg = miny
maxy_avg = maxy
...
for idx in range(len(tests)):
    ...
    for my_batch in batches_to_analyze:
        ...
        stats_path_2 = [base_path + f"/4 D2 quantification/Results/B{my_batch}/B{my_batch}{'C' if my_idx == 0 else 'H'}D2_analysis.txt" for my_idx in range(2)]
        stats_path_3 = [base_path + f"/5 D3 quantification/Results/B{my_batch}/B{my_batch}{'C' if my_idx == 0 else 'H'}D3_analysis.txt" for my_idx in range(2)]

        # Read the file and extract the necessary lists -- either day is fine, using day 3
        for my_ind in range(2):
            c_content = ut.read_file(stats_path_3[my_ind])
            filenames[my_ind] = ut.extract_list(c_content, 'Filenames:')  # Filenames for Day 3

            c_content_2 = ut.read_file(stats_path_2[my_ind])
            filenames_2[my_ind] = ut.extract_list(c_content_2, 'Filenames:')  # Filenames for Day 2
            # Modify Day 2 filenames to have D3 instead of D2
            filenames_2[my_ind] = [filename.replace('D2', 'D3') for filename in filenames_2[my_ind]]

        #### Get common filenames between Day 2 and Day 3
        common_control_filenames = set(filenames[0]).intersection(filenames_2[0])
        common_hypo_filenames = set(filenames[1]).intersection(filenames_2[1])

        common_control_filenames_for_D2 = [filename.replace('D3', 'D2') for filename in common_control_filenames]
        common_hypo_filenames_for_D2 = [filename.replace('D3', 'D2') for filename in common_hypo_filenames]

        newfilenames = [common_control_filenames, common_hypo_filenames]

        if my_units[idx] == 'norm.': 
            # Normalize to mean of Control
            control_data_2 = ut.extract_array_for_common_files(stats_path_2[0], variables[idx], common_control_filenames_for_D2)/np.mean(ut.extract_array_for_common_files(stats_path_2[0], variables[idx], common_control_filenames_for_D2))
            hypo_data_2 = ut.extract_array_for_common_files(stats_path_2[1], variables[idx], common_hypo_filenames_for_D2)/np.mean(ut.extract_array_for_common_files(stats_path_2[0], variables[idx], common_control_filenames_for_D2))

            control_data_3 = ut.extract_array_for_common_files(stats_path_3[0], variables[idx], common_control_filenames)/np.mean(ut.extract_array_for_common_files(stats_path_3[0], variables[idx], common_control_filenames))
            hypo_data_3 = ut.extract_array_for_common_files(stats_path_3[1], variables[idx], common_hypo_filenames)/np.mean(ut.extract_array_for_common_files(stats_path_3[0], variables[idx], common_control_filenames))
        else:
            control_data_2 = ut.extract_array_for_common_files(stats_path_2[0], variables[idx], common_control_filenames_for_D2)
            hypo_data_2 = ut.extract_array_for_common_files(stats_path_2[1], variables[idx], common_hypo_filenames_for_D2)

            control_data_3 = ut.extract_array_for_common_files(stats_path_3[0], variables[idx], common_control_filenames)
            hypo_data_3 = ut.extract_array_for_common_files(stats_path_3[1], variables[idx], common_hypo_filenames)

        #### Calculate the difference, ratio, or percentage change
        concatenated = ut.combine_data(control_data_2, hypo_data_2, control_data_3, hypo_data_3, my_test, concatenated)
        ...
        for my_ind in range(2):
            ...
            # Modify each filename for comparison: include 'D{my_day}' and add '.png' -- either day is fine, using day 3 WILL NOT WORK FOR BATCH > 10
            #modified_filenames[my_ind] = [f"{filename[:3]}D3{filename[3:]}.png" for filename in assess_filenames[my_ind]]
            
            # Modify each filename for comparison: include 'D{my_day}' and add '.png'
            modified_filenames[my_ind] = [
                f"{filename[:filename.index('H') + 1]}D3{filename[filename.index('H') + 1:]}.png"
                if 'H' in filename 
                else f"{filename[:filename.index('C') + 1]}D3{filename[filename.index('C') + 1:]}.png"
                for filename in assess_filenames[my_ind]
                ]

            # Ensure consistent format of filenames before intersection
            modified_filenames_set = set(modified_filenames[my_ind])
            newfilenames_set = set(newfilenames[my_ind])

            # Find common filenames using common filenames for D2 and D3 above, newfilenames
            common_filenames[my_ind] = modified_filenames_set.intersection(newfilenames_set)
            ...
            # Ensure length consistency between concatenated and healthy_indices
            if my_ind == 0:  # Control data
                if len(concatenated[2*(my_batch-1) + my_ind]) >= len(healthy_indices):
                    healthy_concatenated.append([concatenated[2*(my_batch-1) + my_ind][i] for i in healthy_indices])
                    healthy_stickcolors[my_ind].append([stickcolors[my_ind][-1][i] for i in healthy_indices])  # Use stickcolors[my_ind][i] directly
                else:
                    print(f"Warning: concatenated[{2*(my_batch-1) + my_ind}] has fewer elements than healthy_indices!")
            else:  # Hypo data
                if len(concatenated[2*(my_batch-1) + my_ind]) >= len(healthy_indices):
                    healthy_concatenated.append([concatenated[2*(my_batch-1) + my_ind][i] for i in healthy_indices])
                    healthy_stickcolors[my_ind].append([stickcolors[my_ind][-1][i] for i in healthy_indices])  # Use stickcolors[my_ind][i] directly
    ...
    # Debugging final lengths before summation
    ...
    if my_units[idx] != '':
        plt.suptitle(f'a) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0' + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    else:
        plt.suptitle(f'a) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    ...
    if my_units[idx] != '':
        plt.suptitle(f'a) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ only healthy'  + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    else:
        plt.suptitle(f'a) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ only healthy'  + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    ...
    if my_units[idx] != '':
        plt.suptitle(f'b) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    else:
        plt.suptitle(f'b) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    ...
    if my_units[idx] != '':
        plt.suptitle(f'b) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ only healthy'  + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    else:
        plt.suptitle(f'b) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ only healthy'  + my_test_str, fontsize=BIGGER_SIZE, fontproperties=font_title)
    ...
    save_avg_path = base_path + day_path + f'Delta_{tests[idx]}/Batches{batches_str}_{tests[idx]}_Comparison.txt'
    save_avg_path_healthy = base_path + day_path + f'Delta_{tests[idx]}/Batches{batches_str}_{tests[idx]}_Comparison_HEALTHY.txt'
    ...
    if my_units[idx] != '':
        plot_title = f'g) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ all states'  + my_test_str
    else:
        plot_title = f'g) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ all states'  + my_test_str
    my_str = 'K.-W. stat. = ' + "{:.1f}".format(kw_statistic_C) + f' $\cdot$ p value = {p_value_kw_C:.2e}' 
    fig3a, axs3a = plt.subplots()
    ut.plot_dunn_test_matrix_delta(fig3a, axs3a, dunn_results_C, plot_title, font_title, font_text, 
    ...
    ut.plot_dunn_test_matrix_delta(fig3b, axs3b, dunn_results_H, plot_title, font_title, font_text, 
                             SMALL_SIZE, 0.74*MEDIUM_SIZE, 0.7*BIGGER_SIZE, my_str, vmin=global_vmin, vmax=global_vmax, ishypo=True)
    
    if my_units[idx] != '':
        plot_title_h = f'h) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ only healthy'  + my_test_str
    else:
        plot_title_h = f'h) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ only healthy'  + my_test_str
    ...
    ut.plot_dunn_test_matrix_delta(fig4a, axs4a, dunn_results_C_h, plot_title_h, font_title, font_text, 
    ...
    ut.plot_dunn_test_matrix_delta(fig4b, axs4b, dunn_results_H_h, plot_title_h, font_title, font_text, 
    ...
    if my_units[idx] != '':
        plot_title = f'i) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ all states'  + my_test_str
    else:
        plot_title = f'i) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ all states'  + my_test_str
        
    if my_units[idx] != '':
        plot_title_h = f'j) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ only healthy'  + my_test_str
    else:
        plot_title_h = f'j) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ only healthy'  + my_test_str
    ...
    fig5a, axs5a = plt.subplots(figsize=(6, 4.4))
    fig5b, axs5b = plt.subplots(figsize=(6, 4.4))
    ut.plot_tukey_ARTanova_delta(fig5a, axs5a, fig5b, axs5b, tukey_interaction_ARTANOVA, plot_title, font_title, font_text, 
    ...
    fig6a, axs6a = plt.subplots(figsize=(6, 4.4))
    fig6b, axs6b = plt.subplots(figsize=(6, 4.4))
    ut.plot_tukey_ARTanova_delta(fig6a, axs6a, fig6b, axs6b, tukey_interaction_ARTANOVA_h, plot_title_h, font_title, font_text, 
    ...
    for my_first_ind in range(len(concatenated)):
        for my_second_ind in range(my_first_ind + 1, len(concatenated)):
            save_stats_path_batch = base_path + day_path + f'BatchStats/{tests[idx]}_{conditionlabels[my_first_ind]}_{conditionlabels[my_second_ind]}_Comparison.txt'
    ...
    for my_first_ind in range(len(healthy_concatenated)):
        for my_second_ind in range(my_first_ind + 1, len(healthy_concatenated)):
            save_stats_path_batch_healthy = base_path + day_path + f'BatchStats/{tests[idx]}_{conditionlabels[my_first_ind]}_{conditionlabels[my_second_ind]}_Comparison_HEALTHY.txt'
    ...
    if my_units[idx] != '':
        plot_title = f'h) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str
        plot_title_h = f'h) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ only healthy'  + my_test_str
    else:
        plot_title = f'h) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str
        plot_title_h = f'h) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ only healthy'  + my_test_str
    
    fig7a, axs7a = plt.subplots()
    ut.plot_barplot_delta(fig7a, axs7a, f'{tests[idx]}', 
    ...
    ut.plot_barplot_delta(fig7b, axs7b,f'{tests[idx]}', 
    ...
    if my_units[idx] != '':
        plot_title = f'e) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str
        plot_title_h = f'e) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ only healthy'  + my_test_str
    else:
        plot_title = f'e) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str
        plot_title_h = f'e) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ only healthy'  + my_test_str
    ...
    if my_units[idx] != '':
        plot_title = f'f) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ all states\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0'  + my_test_str
        plot_title_h = f'f) Day {my_day} $\cdot$ ' + tests[idx] + f' ({my_units[idx]}) $\cdot$ only healthy'  + my_test_str
    else:
        plot_title = f'f) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ all states'  + my_test_str
        plot_title_h = f'f) Day {my_day} $\cdot$ ' + tests[idx] + ' $\cdot$ only healthy'  + my_test_str
...
###############################################################################
    ...
    output_path = base_path + day_path + f'Delta_{tests[idx]}_{my_test}/'
