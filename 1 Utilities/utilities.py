...
def calculate_percent_diff(concatenated, percentile=5):
    ...
    Returns:
    - result_array: List of percentage differences for each individual list in concatenated.
    ...
    """
    def calculate_diff(data, percentile):
        sorted_data = np.sort(data)
        n = len(sorted_data)
        top_n = int(np.ceil(n * (percentile / 100)))
        ...
        # Calculate percentage difference
        if smallest_avg != 0:
            return ((largest_avg - smallest_avg) / smallest_avg) * 100
        else:
            return float('inf')  # Handle division by zero if smallest average is zero

    # Calculate for individual lists in concatenated
    result_array = [calculate_diff(data, percentile) for data in concatenated]
    ...
    result_comb1 = calculate_diff(combined_1, percentile)

    combined_2 = np.concatenate([concatenated[i] for i in [1, 3, 5, 7, 9, 11, 13]])
    result_comb2 = calculate_diff(combined_2, percentile)

    combined_3 = np.concatenate([concatenated[i] for i in [14, 16, 18]])
    result_comb3 = calculate_diff(combined_3, percentile)

    combined_4 = np.concatenate([concatenated[i] for i in [15, 17, 19]])
    result_comb4 = calculate_diff(combined_4, percentile)
